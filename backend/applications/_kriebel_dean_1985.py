"""Kriebel & Dean (1985) equilibrium-response dune erosion — retained engine, not a
registered CHESS-QC application.

This was CHESS-QC 6-2 until 6-2 was reworked onto the XSHORE finite-difference scheme
that ACES 6-2 actually implements (see chessqc_6_2_dune_erosion.py and
tests/aces_oracle/FINDINGS.md E9). The substitution had been made on the grounds that
the XSHORE source was unavailable; it is available and now runs, so 6-2 follows it.

The file is kept because the model is sound and answers a question XSHORE does not: the
equilibrium recession and response time scale for a design storm surge, closed form and
with no free parameters. It is deliberately named without the `chessqc_` prefix so the
application registry, the docs generator and the validation report do not pick it up.
Nothing imports it; it remains runnable and self-testing on its own:

    python backend/applications/_kriebel_dean_1985.py

Original header follows.

---

Kriebel & Dean (1985) equilibrium-profile erosion model in its analytical, closed-form
limit, which is physically grounded, magnitude-correct, and has zero free parameters.
...
Validated analytically against the source paper: for the paper's Fig-5 case (D50=0.5 mm
so A=0.118, S=2 m, H_b=4.6 m, berm 3 m, slope 1:10) the equilibrium recession is ~79 m
(matching the paper's tens-of-metres and the Bruun balance) and the time scale ~19 hr
(the paper's stated 10-100 hr storm range); recession is linear in surge (paper Fig 5)
